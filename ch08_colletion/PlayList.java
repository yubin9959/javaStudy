...
//		
		...
		//        4. 없으면 ... 아쉽지만 검색결과가 없습니다..(출력)	
		Scanner scan = new Scanner(System.in);
		
		while(true) {
			System.out.println("탑100입니다. 가수와 제목 어떤걸로 알려드릴까요?(종료:q)");
			...
			for (String song : playList) {
				if(song.toLowerCase().contains(userInput.toLowerCase())) {
					System.out.println("순위" + rank + ": " +song);
					found = true;
					rank++;
				}
			}
				if(!found) {
					System.out.println("죄송합니다, 찾으시는 노래를 찾을수 없습니다. '" + userInput + "'.");
				}
	...
	// ouput : 리스트
//	public static ArrayList<String> searchSong(String option, String keyword
